 ...
 * folder management, and direct invocation of Checkmarx Container Realtime
 * scanning via CxWrapperFactory.
 */
public class ContainerScannerService extends BaseScannerService<ContainersRealtimeResults> {
	...
	private static final List<String> CONTAINERS_FILE_PATTERNS = List.of("**/dockerfile*", "**/*.containerfile",
			"**/*.image", "**/docker-compose*.yml", "**/docker-compose*.yaml");

	private static final List<String> CONTAINER_HELM_EXCLUDED_FILES = List.of("chart.yaml", "chart.yml", "values.yaml",
			"values.yml");
	...
	public static ScannerConfig createConfig() {
		return ScannerConfig.builder().engineName(ScanEngine.CONTAINERS.name())
	...
	private boolean isContainersFilePatternMatching(String filePath) {
		...
		List<PathMatcher> pathMatchers = CONTAINERS_FILE_PATTERNS.stream()
				.map(p -> FileSystems.getDefault().getPathMatcher("glob:" + p)).collect(Collectors.toList());
	 ...
	 * structure, executes the container realtime scan, and updates ignored issues.
	 */
